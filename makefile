...
OTHERFLAGS := \
	...
	# -e PGHOST=db.host.name \
	# -e PGPORT=5432 \
	# -e PGSQL_BACKUPDIR=/var/lib/postgresql/backups \
	# -e PGSQL_CUSTOM_CONF=/defaults/postgresql.conf \
	# -e PGSQL_CUSTOM_HBA=/defaults/pg_hba.conf \
	# -e PGSQL_CUSTOM_IDENT=/defaults/pg_ident.conf \
	# -e PGSQL_INITDIR=/initdb.d \
	# -e PGSQL_SKIP_BOOTSTRAP=true \
	# -e PGSQL_SKIP_INITIALIZE=true \
	# -e POSTGRES_DB=test \
	# -e POSTGRES_HOST_AUTH_METHOD=scram-sha-256
	# -e POSTGRES_INITDB_ARGS= \
	# -e POSTGRES_INITDB_WALDIR=/var/lib/postgresql/pg_wal \
	# -e POSTGRES_PASSFILE=/run/s6/container_environment/POSTGRES_PASSWORD \
...
get_config: ## get default config from github
	curl \
		-o $(CURDIR)/root/defaults/postgresql.conf \
		-SL https://github.com/postgres/postgres/raw/refs/heads/REL_$(PGMAJOR)_STABLE/src/backend/utils/misc/postgresql.conf.sample \
	#
# -- }}}
